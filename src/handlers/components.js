...
module.exports = (client) => {
    for (const type of readdirSync('./src/components/')) {
        for (const dir of readdirSync('./src/components/' + type)) {
            for (const subdir of readdirSync('./src/components/' + dir)) {
                for (const file of readdirSync('./src/components/' + type + '/' + dir + '/' + subdir).filter((f) => f.endsWith('.js'))) {
                    const module = require('../components/' + type + '/' + dir + '/' + '/' + subdir + '/' + file);
